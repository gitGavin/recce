...
**The faithfulness failure mode is subtle and interesting.** Both failures are the same pattern: the reviser consolidates or restructures claims from the scout notes in ways that subtly shift attribution or merge separate facts into a single sentence. In the Claude Cowork run, a quote was attributed to Anthropic when the actual source was a third-party blogger commenting on Anthropic's positioning. In the v0 run, two separate facts about the Free tier ($0 price and $5 monthly credits) were merged into one sentence that the judge flagged as a fabrication. This is not hallucination from nothing — it's lossy compression of real information. The distinction matters because the fix is different: hallucination requires guardrails, but lossy compression requires stricter paraphrasing constraints in the reviser prompt.

**The judge had a false positive.** Manual validation of the v0 run showed the judge was wrong on one faithfulness call. The "$5 monthly credits" claim was in the scout notes, just split across two sentences. The judge read the consolidation as fabrication. This is a known limitation of LLM-as-judge — it struggles when source information is distributed and the output consolidates it. A more robust eval would check semantic equivalence rather than near-exact textual matching. Catching this false positive is itself the point of judge validation — you cannot trust automated evals without spot-checking them against human judgment.

**Binary pass/fail was the right call.** If I had used a 1-5 scale, the faithfulness failures would have been 3s or 4s — "mostly faithful with minor issues." That ambiguity hides the problem. Binary forced a clear verdict: the reviser either preserved attribution accurately or it didn't. This made the failure pattern obvious in the summary table. Hamel Husain's advice on binary evals is correct — the clarity is worth the loss of granularity.
